...
local heartbeatActions = {
	...
	["localPlayer"] = function(recommendedDetection)
		...
		local touchingZones = ZoneController.getTouchingZones(
			character,
			true,
			recommendedDetection,
			trackers.player
...
function ZoneController._registerConnection(
	registeredZone,
	...
	activeTriggers[registeredTriggerType] = (
		currentTriggerCount and currentTriggerCount + 1
	) or 1
...
function ZoneController._formHeartbeat(registeredTriggerType)
	...
	heartbeatConnection = heartbeat:Connect(function()
		...
		if clockTime >= nextCheck then
			...
			local zonesAndOccupants =
				heartbeatActions[registeredTriggerType](lowestDetection)
			...
			for zone, newOccupants in pairs(zonesAndOccupants) do
				local settingsGroup = (
					...
					and ZoneController.getGroup(
						zone.settingsGroupName
					)
				)
				if
					settingsGroup
					and settingsGroup.onlyEnterOnceExitedAll
						== true
				then
					...
					for newOccupant, _ in pairs(newOccupants) do
						...
						local groupDetail =
							occupantsToBlock[zone.settingsGroupName]
			...
			for zone, newOccupants in pairs(zonesToPotentiallyIgnore) do
				...
				if groupDetail then
					for newOccupant, _ in pairs(newOccupants) do
						local occupantToKeepZone =
							groupDetail[newOccupant]
						if
							occupantToKeepZone
							and occupantToKeepZone ~= zone
						then
			...
			for zone, _ in pairs(activeZones) do
				if zone.activeTriggers[registeredTriggerType] then
					local zAccuracy = zone.accuracy
					local occupantsDict = zonesAndOccupants[zone]
						or {}
					...
					if
						occupantsPresent
						and zAccuracy > highestAccuracy
					then
						highestAccuracy = zAccuracy
					end
					local signalsToFire = zone:_updateOccupants(
						registeredTriggerType,
						occupantsDict
					)
					collectiveSignalsToFire[1][zone] =
						signalsToFire.exited
					collectiveSignalsToFire[2][zone] =
...
function ZoneController._deregisterConnection(
	registeredZone,
	registeredTriggerType
)
	activeConnections -= 1
	if activeTriggers[registeredTriggerType] == 1 then
		...
		local heartbeatConnection =
			heartbeatConnections[registeredTriggerType]
		if heartbeatConnection then
	...
	if tracker.totalVolume < totalZoneVolume then
		...
		for _, item in pairs(tracker.items) do
			...
			for _, zone in pairs(touchingZones) do
				if
					not onlyActiveZones
					or zone.activeTriggers[trackerName]
				then
					local finalItem = item
					if trackerName == "player" then
						finalItem =
							players:GetPlayerFromCharacter(item)
					end
					if finalItem then
						fillOccupants(
							zonesAndOccupants,
		...
		for zone, _ in pairs(zonesDictToCheck) do
			if
				not onlyActiveZones or zone.activeTriggers[trackerName]
			then
				...
				for _, itemOrChild in pairs(result) do
					local correspondingItem =
						tracker.partToItem[itemOrChild]
...
function ZoneController.getTouchingZones(
	item,
	onlyActiveZones,
	recommendedDetection,
	tracker
	...
	local partToZoneDict = (onlyActiveZones and activePartToZone)
		or allPartToZone
	...
	local zonesDict = {}
	local boundParts = CollectiveWorldModel:GetPartBoundsInBox(
		itemCFrame,
		itemSize,
		boundParams
	)
	...
	for _, boundPart in pairs(boundParts) do
		...
		if
			correspondingZone and correspondingZone.allZonePartsAreBlocks
		then
	...
	if totalRemainingBoundParts > 0 then
		...
		preciseParams.FilterDescendantsInstances =
			boundPartsThatRequirePreciseChecks

		local character = item
		for _, bodyPart in pairs(bodyPartsToCheck) do
			local endCheck = false
			if
				not bodyPart:IsA("BasePart")
				or (
					itemIsCharacter
					and Tracker.bodyPartsToIgnore[bodyPart.Name]
				)
			then
				continue
			end
			local preciseParts = CollectiveWorldModel:GetPartsInPart(
				bodyPart,
				preciseParams
			)
			for _, precisePart in pairs(preciseParts) do
				if not touchingPartsDictionary[precisePart] then
					local correspondingZone =
						partToZoneDict[precisePart]
					if correspondingZone then
						zonesDict[correspondingZone] = true
						touchingPartsDictionary[precisePart] =
							correspondingZone
						precisePartsCount += 1
					end
					if
						precisePartsCount
						== totalRemainingBoundParts
					then
						endCheck = true
						break
					end
	...
	for zone, _ in pairs(zonesDict) do
		if
			newExitDetection == nil
			or zone._currentExitDetection < newExitDetection
		then
...
local workspaceContainerName = string.format(
	"ZonePlus%sContainer",
	(runService:IsClient() and "Client") or "Server"
)
function ZoneController.getWorkspaceContainer()
	local container = workspaceContainer
		or workspace:FindFirstChild(workspaceContainerName)
