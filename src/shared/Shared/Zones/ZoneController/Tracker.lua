...
function Tracker.getCharacterSize(character)
	...
	local charSize = (hrpSize * Vector3.new(2, 2, 1))
		+ Vector3.new(0, headY, 0)
	local charCFrame = hrp.CFrame
		* CFrame.new(0, headY / 2 - hrpSize.Y / 2, 0)
		...
		local function playerAdded(player)
			local function charAdded(character)
				...
				if humanoid then
					...
					self:update()
					for _, valueInstance in
						pairs(humanoid:GetChildren())
					do
						if valueInstance:IsA("NumberValue") then
							valueInstance.Changed:Connect(
								function()
									self:update()
								end
							)
...
function Tracker:update()
	...
	for character, _ in pairs(self.characters) do
		...
		local characterJanitor = self.janitor:add(
			Janitor.new(),
			"destroy",
			"trackCharacterParts-" .. self.name
		)
		...
		for _, part in pairs(character:GetChildren()) do
			if
				part:IsA("BasePart")
				and not Tracker.bodyPartsToIgnore[part.Name]
			then
