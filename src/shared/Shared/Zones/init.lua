...
local referenceLocation = (
	game:GetService("RunService"):IsClient() and "Client"
) or "Server"
local referencePresent = referenceObject
	and referenceObject:FindFirstChild(referenceLocation)
...
function Zone.new(container)
	...
	-- Validate container
	local INVALID_TYPE_WARNING =
		"The zone container must be a model, folder, basepart or table!"
	...
	for _, triggerType in pairs(triggerTypes) do
		...
		for i, triggerEvent in pairs(triggerEvents) do
			...
			local triggerEventUpper = triggerEvent:sub(1, 1):upper()
				.. triggerEvent:sub(2)
			local signalName = triggerType .. triggerEventUpper
			self[signalName] = signal
			signal.connectionsChanged:Connect(function(increment)
				if
					triggerType == "localPlayer"
					and not localPlayer
					and increment == 1
				then
					error(
						("Can only connect to 'localPlayer%s' on the client!"):format(
							triggerEventUpper
						)
					)
				end
				...
				if
					previousActiveConnections == 0
					and activeConnections > 0
				then
					-- At least 1 connection active, begin loop
					ZoneController._registerConnection(
						self,
						triggerType,
						triggerEventUpper
					)
				...
				then
					...
					ZoneController._deregisterConnection(
						self,
						triggerType
					)
				end
	...
	for _, triggerType in pairs(triggerTypes) do
		...
		if correspondingMethod then
			...
			Zone.touchedConnectionActions[triggerType] = function(
				touchedItem
			)
...
function Zone.fromRegion(cframe, size)
	...
	local function createCube(cubeCFrame, cubeSize)
		if
			cubeSize.X > MAX_PART_SIZE
			or cubeSize.Y > MAX_PART_SIZE
			or cubeSize.Z > MAX_PART_SIZE
		then
...
function Zone:_calculateRegion(tableOfParts, dontRound)
	...
	for _, part in pairs(tableOfParts) do
		...
		local corners = {
			part.CFrame
				* CFrame.new(-sizeHalf.X, -sizeHalf.Y, -sizeHalf.Z),
			part.CFrame
				* CFrame.new(-sizeHalf.X, -sizeHalf.Y, sizeHalf.Z),
			part.CFrame
				* CFrame.new(-sizeHalf.X, sizeHalf.Y, -sizeHalf.Z),
...
function Zone:_displayBounds()
	if not self.displayBoundParts then
		...
		local boundParts =
			{ BoundMin = self.boundMin, BoundMax = self.boundMax }
	...
	for _, part in pairs(zoneParts) do
		for _, prop in pairs(partProperties) do
			self._updateConnections:add(
				part:GetPropertyChangedSignal(prop):Connect(update),
				"Disconnect"
			)
		end
		verifyDefaultCollision(part)
		self._updateConnections:add(
			part:GetPropertyChangedSignal("CollisionGroupId")
				:Connect(function()
					verifyDefaultCollision(part)
				end),
			"Disconnect"
...
function Zone:_updateOccupants(trackerName, newOccupants)
	...
	for occupant, _ in pairs(newOccupants) do
		if previousOccupants[occupant] == nil then
			...
			previousOccupants[occupant] = (
				isAPlayer and occupant.Character
			) or true
	...
	for _, basePart in pairs(self.zoneParts) do
		touchedJanitor:add(
			basePart.Touched:Connect(
				self.touchedConnectionActions[triggerType],
				self
			),
			"Disconnect"
		)
...
function Zone:_partTouchedZone(part)
	...
	if
		not (
			instanceClassesToIgnore[part.ClassName]
			or not instanceNamesToIgnore[part.Name]
		)
	then
		part.CanTouch = false
	end
	...
	partJanitor:add(
		heartbeat:Connect(function()
			...
			if clockTime >= nextCheck then
				----
				local cooldown =
					enum.Accuracy.getProperty(self.accuracy)
				...
				if not verifiedEntrance then
					...
					elseif
						(part.Position - enterPosition).Magnitude
							> 1.5
						and clockTime - enterTime >= cooldown
					then
...
function Zone:findPart(part)
	local methodName, args = self:_getRegionConstructor(
		part,
		self.overlapParams.zonePartsWhitelist
	)
	local touchingZoneParts =
		self.worldModel[methodName](self.worldModel, unpack(args))
...
function Zone:getParts()
	...
	local partsInRegion = self.worldModel:GetPartBoundsInBox(
		self.region.CFrame,
		self.region.Size,
...
function Zone:trackItem(instance)
	...
	if not isBasePart then
		isCharacter = instance:FindFirstChildOfClass("Humanoid")
			and instance:FindFirstChild("HumanoidRootPart")
	end

	assert(
		isBasePart or isCharacter,
		"Only BaseParts or Characters/NPCs can be tracked!"
	)
...
function Zone:bindToGroup(settingsGroupName)
	...
	local group = ZoneController.getGroup(settingsGroupName)
		or ZoneController.setGroup(settingsGroupName)
...
function Zone:relocate()
	...
	self.relocationContainer = self.janitor:add(
		relocationContainer,
		"Destroy",
		"RelocationContainer"
	)
