...
function PhysicsGrab.new(Character)
    ...
    self.AlignPosition.Attachment1 =  self.Attachment
...
function PhysicsGrab:Hold(object)
    if self:IsGrabbing() then self:Unhold() return end
...
function PhysicsGrab:Update(dt)
    self.CameraPart.Position = Camera.CFrame.LookVector * 4 + self.Head.Position
    self.CameraPart.Orientation = self.CameraPart.Orientation
    if not self.Grabbed then return end
    self.AlignOrientation.CFrame = CFrame.lookAt(self.Grabbed.Position, self.Head.Position)
end
